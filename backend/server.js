...
app.get('/api/products-features', async (req, res) => {
    try {
        const query = `
            SELECT p.*, f.price as feature_price, f.weight, f.cushioning_level, f.arch_support, f.foot_width_support, f.shoe_type, f.upper_material, f.sole_material, f.breathability, f.durability, COALESCE(AVG(pr.rating), 0) AS avg_rating
            FROM products p
            LEFT JOIN features f ON p.id = f.product_id
            LEFT JOIN product_review pr ON p.id = pr.product_id
            GROUP BY p.id, f.price, f.weight, f.cushioning_level, f.arch_support, f.foot_width_support, f.shoe_type, f.upper_material, f.sole_material, f.breathability, f.durability
        `;
